...
\begin{theorem}[Decidability for Bidirectional Typing Synthesis] \label{thm:bidirectional-type-synthesis}
  ...
  \begin{enumerate}
    \item For every context $\Gamma$ and a raw term $\isTerm{t}$ with a synthesis mode, it is decidable that there exists a type $A$ and a derivation of
      \[
        \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} \Rightarrow A.
      \]
    \item For every context $\Gamma$, a raw term $\isTerm{t}$ with a checking mode, and a type $A$, it is decidable that there is a derivation of
      \[
...
\begin{corollary}[`Decidability' of Type Synthesis]
  \LT{Still call it `decidability'?}
  ...
  \begin{enumerate}
    \item there is a typing derivation of $\Gamma \vdash_{\Sigma, \Omega} t : A$ for some type $A$.
...
\subsection{Proof for Decidability of Bidirectional Type Synthesis} 

\begin{theorem}[Bidirectional Type Synthesis and Checking]
...
%      \]
  ...
  \end{enumerate}
\end{theorem}
By induction on raw terms $t$ with a mode and on the list of arguments for operations. 
