...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \and
    \inferrule{\Theta \mid \Gamma, \isTerm{x} : A \vdash \isTerm{t} \Rightarrow B}{\Theta \mid \Gamma \vdash \isTerm{\lam{x}t} \Rightarrow A \bto B}
  \end{mathpar}
...
Intrinsically typed terms for a type signature $\Sigma$ and a term signature $\Omega$ are nothing more than derivations of the intrinsic typing judgement $\boxed{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} A}$ constructed with only the rule $(\var)$ and the rule scheme $(\termOp)$ displayed in \Cref{fig:intrinsic-typing}.
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \and
    \inferrule*{\rho : \Sub{\Xi}{\Theta} \\ \Theta \mid \Gamma, \simsub{\Delta_{1}}{\rho} \vdash_{\Sigma, \Omega} \simsub{A_{1}}{\rho} \\ \cdots \\ \Theta \mid \Gamma, \simsub{\Delta_{n}}{\rho} \vdash_{\Sigma, \Omega} \simsub{A_{n}}{\rho}}
    {\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \simsub{A}{\rho}}\;(\termOp)
  \end{mathpar}
...
\begin{figure}
  ...
  \small
  \begin{mathpar}
    \boxed{\Theta \vdash_{\Sigma, \Omega} t^d}
  \end{mathpar}
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \and
    \inferrule{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : B^\chk \\ A = B}{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t : B}:  A^\syn}\;(\text{anno})
    \and
  ...
  \end{mathpar}
  \begin{mathpar}
    \inferrule{\rho\colon \Sub{\Xi}{\Theta} \\
      \Theta \mid \Gamma, \simsub{\isTerm{\vec{x}_{1}} : \Delta_{1}}{\rho} \vdash_{\Sigma, \Omega} \isTerm{t_{1}} : \simsub{A_{1}}{\rho}^{\isDir{d_1}} \\
      \cdots \\
    \Theta \mid \Gamma, \simsub{\isTerm{\vec{x}_{n}} : \Delta_{n}}{\rho} \vdash_{\Sigma, \Omega} \isTerm{t_n} : \simsub{A_{n}}{\rho}^{\isDir{d_n}}}
  {\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{\termOp_o(\vec{x}_1 .\, t_{1}; \ldots; \vec{x}_{n}.\, t_{n})} : \simsub{A}{\rho}^{\isDir{d}}} \; (\termOp)
    \\ \text{for $o \colon \Xi \vdash [\Delta_1]A_{1}^{d_1}, \ldots, [\Delta_{n}] A^{d_n}_{n} \to A^{d}$ in $\Omega$}
  \end{mathpar}
  Abbreviations: $\boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} \Rightarrow A} \defeq \boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : A^\syn}$ and $\boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} \Leftarrow A} \defeq \boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : A^\chk}$
