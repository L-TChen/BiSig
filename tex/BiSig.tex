...
\begin{theorem}
  ...
  \begin{enumerate}
    ...
    \item For every set $\Xi$ of type variables, a list $\Delta$ of types with variables in $\Xi$, a partial substitution $\rho\colon \PSub{\Xi, \emptyset}$ defined for all types in $\Delta$, and a raw term with mode $d$ then 
      \begin{enumerate}
        \item if $d$ is $\Rightarrow$, then it is decidable that there exists a type $A$ and a typing derivation of
          \[
            \Gamma, \simsub{\Delta}{\rho} \vdash_{\Sigma, \Omega} t \Rightarrow A 
          \]
        \item if $d$ is $\Leftarrow$, then for every type $A$ it is decidable that there is a typing derivation of 
          \[
            \Gamma, \simsub{\Delta}{\rho} \vdash_{\Sigma, \Omega} t \Leftarrow A.
          \]
      \end{enumerate}
  \end{enumerate}
\end{theorem}
