...
\begin{figure}
  ...
  \small
  \begin{mathpar}
    \boxed{\Theta \mid \Gamma \vdash \isTerm{t} \Rightarrow A}
    \quad\text{and}\quad \boxed{\Theta \mid \Gamma \vdash \isTerm{t} \Leftarrow A}
    \quad\text{where $\Theta \vdash \Gamma$ and $\Theta \vdash A$} 
    \\
    \inferrule{(x : A) \in \Gamma}{\Theta \mid \Gamma \vdash \isTerm{x} \Rightarrow A}\;(\text{var}^\Rightarrow)
    \and
    \inferrule{\Theta \mid \Gamma \vdash \isTerm{t} \Leftarrow B \\ A = B}{\Theta \mid \Gamma \vdash \isTerm{t : B}\Rightarrow A}\;(\text{anno})
    \and
    \inferrule{\Theta \mid \Gamma \vdash \isTerm{t} \Rightarrow B \\ A = B}{\Theta \mid \Gamma \vdash \isTerm{t\uparrow} \Leftarrow A}\;(\text{sub})
    \and
    \inferrule{\Theta \mid \Gamma \vdash \isTerm{t} \Rightarrow A \bto B \\ \Theta \mid \Gamma \vdash \isTerm{u} \Leftarrow A}{\Theta \mid \Gamma \vdash \isTerm{t\;u} \Rightarrow B}
    \and
    \inferrule{\Theta \mid \Gamma, \isTerm{x} : A \vdash \isTerm{t} \Leftarrow B}{\Theta \mid \Gamma \vdash \isTerm{\lam{x}t} \Leftarrow A \bto B}
  \end{mathpar}
...
Intrinsically typed terms for a type signature $\Sigma$ and a term signature $\Omega$ are nothing more than derivations of the intrinsic typing judgement $\boxed{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} A}$ constructed with only the rule $(\var)$ and the rule scheme $(\tmOp)$ displayed in \Cref{fig:intrinsic-typing}.
...
\begin{figure}
  ...
  \small
  \begin{mathpar}
    \boxed{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} A} \quad \text{where $\Theta \vdash_{\Sigma} \Gamma$ and $\Theta \vdash_{\Sigma} A$} \\
    \inferrule{A \in \Gamma}{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} A}\;(\var)
    \and
    \inferrule{\Theta \mid \Gamma, \simsub{\Delta_{1}}{\rho} \vdash_{\Sigma, \Omega} \simsub{A_{1}}{\rho} \quad\cdots\quad \Theta \mid \Gamma, \simsub{\Delta_{n}}{\rho} \vdash_{\Sigma, \Omega} \simsub{A_{n}}{\rho}}
    {\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \simsub{A}{\rho}}\;(\tmOp)
    \and \text{for $o\colon \Xi \vdash [\Delta_1]A_1, \ldots, [\Delta_{n}]A_{n} \to A$ in $\Omega$
    and $\rho : \Sub{\Xi}{\Theta}$}
  \end{mathpar}
...
\begin{figure}
  ...
  \small
  \begin{mathpar}
    \boxed{\Theta \vdash_{\Sigma, \Omega} \isTerm{t}}
    \\
    \inferrule{x : \Identifier}{\Theta \vdash_{\Sigma, \Omega} \isTerm{x}}\;(\text{var})
    \and
    \inferrule{\Theta \vdash A \\ \Theta \vdash_{\Sigma, \Omega}\isTerm{t}}{\Theta \vdash_{\Sigma, \Theta} \isTerm{t \annote A}}\;(\text{anno})
    \\
    \inferrule{\Theta \vdash_{\Sigma, \Omega} \isTerm{t_1} \quad \cdots \quad \Theta \vdash_{\Sigma, \Omega} \isTerm{t_n}}
    {\Theta \vdash_{\Sigma, \Omega} \isTerm{\tmOp_o(t_1, \ldots, t_n)}}\;(\text{op}) 
...
\begin{figure}
  ...
  \small
  \begin{mathpar}
    \boxed{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : A} \quad \text{where $\Theta \vdash_{\Sigma} \Gamma$, $\Theta \vdash_{\Sigma} A$, and $\Theta \vdash_{\Sigma, \Omega} t$} \\
    \inferrule{(x : A) \in \Gamma}{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{x} : A}\;(\var)
    \and
    \inferrule{\Theta \mid \Gamma \vdash \isTerm{t} : A}{\Theta \mid \Gamma \vdash (\isTerm{t \annote A}) : A}\;(\text{anno})
    \and
    \inferrule{\Theta \mid \Gamma, \isTerm{\vec{x}_1} : \simsub{\Delta_{1}}{\rho} \vdash_{\Sigma, \Omega} \isTerm{t_1} : \simsub{A_{1}}{\rho} \quad\cdots\quad \Theta \mid \Gamma, \isTerm{\vec{x}_n} : \simsub{\Delta_{n}}{\rho} \vdash_{\Sigma, \Omega} \isTerm{t_n} : \simsub{A_{n}}{\rho}}
    {\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{\tmOp_o(\vec{x}_1.\,t_1; \ldots; \vec{x}_n.\,t_n)} : \simsub{A}{\rho}}\;(\tmOp)
    \and \text{for $o\colon \Xi \vdash [\Delta_1]A_1, \ldots, [\Delta_{n}]A_{n} \to A$ in $\Omega$
    and $\rho : \Sub{\Xi}{\Theta}$}
  \end{mathpar}
...
\begin{figure}
  ...
  \small
  \begin{mathpar}
    \boxed{\Theta \vdash_{\Sigma, \Omega} \isTerm{t}\;\isDir{d}}
    \\
    \inferrule{x : \Identifier}{\Theta \vdash_{\Sigma, \Omega} \isTerm{x}\;\isDir{\syn}}\;(\text{var})
    \and
    \inferrule{\Theta \vdash A \\ \Theta \vdash_{\Sigma, \Omega}\isTerm{t}\;\isDir{\chk}}{\Theta \vdash_{\Sigma, \Theta} \isTerm{t \annote A}\;\isDir{\syn}}\;(\text{anno})
    \and
%    \inferrule{\Theta \vdash_{\Sigma, \Omega}\isTerm{t}\;\isDir{\chk}}{\Theta, X \vdash_{\Sigma, \Theta} \isTerm{t \downarrow}\;\isDir{\syn}}\;(\text{anno}')
%    \and
    \inferrule{\Theta \vdash_{\Sigma, \Omega} \isTerm{t}\;\isDir{\syn}}{\Theta \vdash_{\Sigma, \Omega} (\isTerm{t \uparrow})\;\isDir{\chk}}
  \end{mathpar}
  \begin{mathpar}
    \inferrule{\rho \colon \Sub{\Xi}{\Theta} \\ \Theta \vdash_{\Sigma, \Omega} \isTerm{t_1}\;\isDir{d_1} \quad \cdots \quad \Theta \vdash_{\Sigma, \Omega} \isTerm{t_n}\;\isDir{d_n}}
    {\Theta \vdash_{\Sigma, \Omega} \isTerm{\tmOp_o(\rho; \vec{x}_1.\, t_1; \ldots;\vec{x}_n.\, t_n)}\;\isDir{d}}\;(\text{op})
    \and \text{for $o \colon \Xi \vdash [\Delta_1]A_{1}^{d_1}, \ldots, [\Delta_{n}] A^{d_n}_{n} \to A^{d}$ in $\Omega$}
  \end{mathpar}
  
  \caption{Bidirectional pre-typed terms}
\end{figure}
\begin{figure}
  \centering
  \small
  \begin{mathpar}
    \boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : A^\isDir{d}}
    \quad \text{where $d$ is either $\chk$ or $\syn$, $\Theta \vdash_{\Sigma} \Gamma$, $\Theta \vdash_{\Sigma} A$, and $\Theta \vdash_{\Sigma, \Omega} t\;d$} 
    \\
    \inferrule{(x : A) \in \Gamma}{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{x} : A^\syn}\;(\var)
    \and
    \inferrule{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : B^\chk \\ A = B}{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} (\isTerm{t \annote B}):  A^\syn}\;(\text{anno})
    \and
    \inferrule{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : B^{\syn} \\ A = B}{\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t\uparrow} : A^\chk}\;(\text{sub})
    \\
    \inferrule{\rho\colon \Sub{\Xi}{\Theta} \\ \Theta \mid \Gamma, \simsub{\isTerm{\vec{x}_{1}} : \Delta_{1}}{\rho} \vdash_{\Sigma, \Omega} \isTerm{t_{1}} : \simsub{A_{1}}{\rho}^{\isDir{d_1}} \\
      \cdots \\
    \Theta \mid \Gamma, \simsub{\isTerm{\vec{x}_{n}} : \Delta_{n}}{\rho} \vdash_{\Sigma, \Omega} \isTerm{t_n} : \simsub{A_{n}}{\rho}^{\isDir{d_n}}}
  {\Theta \mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{\tmOp_o(\rho; \vec{x}_1 .\, t_{1}; \ldots; \vec{x}_{n}.\, t_{n})} : \simsub{A}{\rho}^{\isDir{d}}} \; (\tmOp)
    \and \text{for $o \colon \Xi \vdash [\Delta_1]A_{1}^{d_1}, \ldots, [\Delta_{n}] A^{d_n}_{n} \to A^{d}$ in $\Omega$}
    \\
    \text{Abbreviations: $\boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} \Rightarrow A} \defeq \boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : A^\syn}$ and $\boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} \Leftarrow A} \defeq \boxed{\Theta\mid \Gamma \vdash_{\Sigma, \Omega} \isTerm{t} : A^\chk}$}
  \end{mathpar}
