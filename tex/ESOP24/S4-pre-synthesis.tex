...
We also discuss some related properties: by bringing mode derivations into the picture, we are able to give a natural formulation of soundness and completeness of a bidirectional type system with respect to its erasure to an ordinary type system in \cref{sec:soundness-and-completeness}.
...
In fact we will do better: if a mode decorator returns a proof that no mode derivation exists, that proof (of a negation) does not provide useful information for the user.
...
\end{proposition}
\LT{the paper is perhaps too modest: (...) I don't think DK annotatability says anything about evidence---a synthesiser that answered "no" could still enjoy annotatability (even if we would not enjoy that answer).}
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \and
    \inferrule{\isTerm{t} \sqsupseteq \isTerm{u}}
      {(\isTerm{t} \annotate A) \sqsupseteq (\isTerm{u} \annotate A)}\;\Rule{Anno}
    \and
