...
%              \text{the domain of $\ext{\rho}$ contains $\synvar(\biargs)$ and\/} \quad
      ...
      \begin{itemize}
        ...
        \item $\dir{d}$ is $\chk$: Let $A$ be a type and apply \Cref{lem:unify} with $\rho_0$ defined nowhere.
          ...
          \end{enumerate}
      \end{itemize}
      We have proved the decidability by induction on the derivation $\erase{\Gamma} |-_{\Sigma, \Omega} t^{\dir{d}}$, assuming the claim which remains to be proved as follows.
