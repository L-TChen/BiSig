        ...
        \item $\dir{d}$ is $\chk$: Let $A$ be a type and apply \Cref{lem:unify} with $\rho_0$ defined nowhere.
          \begin{enumerate}
            ...
            \item If there is a minimal $\ext{\rho} \geq \rho_0$ s.t.\ $\simsub{A_0}{\ext{\rho}} = A$, then apply our claim with $\ext{\rho}$:
              \begin{enumerate}
                \item If there is no $\sigma \geq \ext{\rho}$ satisfying \eqref{eq:claim} but $\Gamma |- \tmOpts \chk A$, then by inversion there is $\gamma$ s.t.\ $\simsub{A_0}{\gamma} = A$ and ${\Gamma, \vars{x}_\isTerm{i} : \simsub{\Delta_i}{\gamma} |- \isTerm{t_i} \colon \simsub{A_i}{\gamma}^{\dir{d_i}}}$ for every $i$.
