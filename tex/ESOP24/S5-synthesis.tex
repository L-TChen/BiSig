...
\begin{lemma}\label{lem:decidability-mode-correctness}
  For any operation $\biop$, it is decidable whether $o$ is mode-correct.
\end{lemma}
...
%              \text{the domain of $\ext{\rho}$ contains $\synvar(\biargs)$ and\/} \quad
      ...
      \begin{itemize}
        ...
        \item $\dir{d}$ is $\chk$: Let $A$ be a type and apply \Cref{lem:unify} with $\rho_0$ defined nowhere.
          \begin{enumerate}
            \item If there is no $\sigma \geq \rho_0$ s.t.\ $\simsub{A_0}{\sigma} = A$ but $\Gamma |- \tmOpts \chk A$, then by inversion there is a substitution $\rho$ such that $A = \simsub{A_0}{\rho}$, thus a contradiction.
