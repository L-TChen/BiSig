...
\begin{definition}
  ...
  For an argument list $\biargs$, its set of \emph{synthesised type variables} is denoted by $\synvar(\biargs)$.
\end{definition}
...
It is easy to check the bidirectional type system $(\Sigma_{\bto}, \Omega^{\Leftrightarrow}_{\Lambda}$) in \Cref{ex:signature-simply-typed-lambda} for simply typed $\lambda$-calculus is mode-correct manually by definition or automatically by the following lemma:
\begin{lemma}
  It is decidable if an operation $\biop$ is mode-correct or not.
\end{lemma}
...
%              \text{the domain of $\ext{\rho}$ contains $\synvar(\biargs)$ and\/} \quad
      ...
      \end{proof}
%  \end{itemize}
  We have completed the proof by induction on the mode derivation $\erase{\Gamma} |-_{\Sigma, \Omega} t^{\dir{d}}$.\qed
\end{proof}
