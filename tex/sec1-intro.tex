...
Our theory can be viewed as a counterpart to those theories in \Cref{sec:theory-of-syntax} and is tailored towards \emph{extrinsically-typed}, \emph{syntax-directed} bidirectional simple type systems (\Cref{fig:raw-terms,fig:bidirectional-typing-derivations}) specified by a \emph{signature} for simple types and a \emph{bidirectional binding signature} for terms (\Cref{def:simple-signature,def:bidirectional-binding-signature}), c.f.\ \Cref{re:type-signature}. 
...
The core technical contribution lies in \Cref{sec:type-synthesis} where we introduce mode-correctness and bidirectional type synthesis.
