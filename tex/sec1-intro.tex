...
As \citet{Pierce2000} noted, bidirectional type synthesis propagates type information locally within adjacent nodes of a raw term, and does not require unification as Damas--Milner type inference does.
...
Despite sharing the same basic idea, bidirectional typing has only been developed on a case-by-case basis without a theory; this situation is in stark contrast to parsing, which has a comprehensive theory together with widely applicable techniques and practical tools, notably parser generators.%
...
To remedy the situation, we initiate a theory of bidirectional typing, which amounts to a verified type-synthesiser generator for syntax-directed bidirectional simple type systems.
...
Recall that the law of excluded middle $P + \neg P$ does not hold as an axiom for every $P$ in Martin-L\"of type theory and we say $P$ is \emph{decidable} if it holds for $P$.
For example, a proof of \emph{decidable equality}, i.e. $\forall x, y.~(x = y) + (x \neq y)$, decides whether they are equal and gives an identity proof explicitly if they are, which may or may not be possible depending on the domain of $x$~and~$y$.
...
We sketch the design of our formalisation in \cref{sec:formalisation} briefly.
We conclude in \cref{sec:future}, where we reflect on techniques that may improve the formal development and potential challenges in extending our theory to a more general setting.
