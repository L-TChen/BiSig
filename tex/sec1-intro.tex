...
Hence, our decidable bidirectional type synthesis (\Cref{thm:bidirectional-type-synthesis-checking}) encapsulates algorithmic soundness, completeness, and decidability simultaneously. 
...
To clarify the difference (\Cref{sec:annotatability}), we propose \emph{mode preprocessing} (\Cref{sec:mode-preprocessing}), which assigns a mode to a raw term with enough annotations, or pinpoints missing type annotations otherwise.
