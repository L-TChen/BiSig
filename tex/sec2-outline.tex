...
\begin{figure}
  ...
  \path[->]
      (m-1-1) edge node [above,align=center] {mode preprocessing} (m-1-2)
              edge node [left,align=left]  {type\\synthesis} (m-2-1)
      (m-1-2) edge node [right,align=right] {bidirectional\\type synthesis} (m-2-2)
