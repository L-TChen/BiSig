...
\Josh{Here we briefly recap the basic ideas of bidirectional type synthesis.
...
\begin{figure}
  \centering
  \small
  \judgbox{V |- \isTerm{t}^\dir{d}}{A raw term~$\isTerm{t}$ is in mode~$d$ with free variables in $V$}
  \begin{mathpar}
...
If\/ $|\Gamma| \vdash t^\dir{\syn}$ and\/ $\Gamma \vdash t : A$, then $\Gamma \vdash t \syn A$.
\end{lemma}
