...
Parametrised by an `excuse' predicate~$E$ on raw terms, a \emph{type synthesiser} takes a context~$\Gamma$ and a raw term $\erase\Gamma \vdash t$ (where $\erase\Gamma$~is the domain of~$\Gamma$) as input, and establishes one of the following outcomes:
\begin{enumerate}
...
A \emph{mode preprocessor} decides for any raw term~$V \vdash \isTerm{t}$ whether $V \vdash \isTerm{t}^{\syn}$.
\end{definition}

One (less important) benefit of mode preprocessing is that it helps to simplify the synthesiser, whose computation can be partly directed by a mode derivation.
...
Run the mode preprocessor on the input term~$\erase\Gamma \vdash t$.
...
If\/ $\Gamma \vdash t \syn A$, then $\Gamma \vdash t : A$.
\end{lemma}
...
If\/ $\erase\Gamma \vdash t^{\syn}$ and\/ $\Gamma \vdash t : A$, then $\Gamma \vdash t \syn A$.
\end{lemma}
...
%      (m-1-1) edge node [above] {mode preprocessing} (m-1-2)
