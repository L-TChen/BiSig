...
\begin{figure}
  \begin{minipage}[b]{.6\textwidth}
    \centering
    \small
    \judgbox{\Theta|-_{\Sigma} A}{$A$ is a well-formed type with type variables in $\Theta$}
    \begin{mathpar}
      \inferrule{\Theta \ni X_i}{\Theta |-_\Sigma X_i} \and
      \inferrule{\Theta |-_{\Sigma} A_1 \\ \cdots \\ \Theta |-_{\Sigma} A_n}{\Theta |-_{\Sigma} \tyOp_o(A_1, \ldots, A_n)}\;\text{where $n = \arity(o)$}
    \end{mathpar}
...
\begin{figure}
  \centering
  \small
  \judgbox{|-_{\Sigma, \Omega} \isTerm{t}}{$\isTerm{t}$ is a raw term for the signature $(\Sigma, \Omega)$}
  \begin{mathpar}
    \inferrule{\isTerm{x} : \Identifier}{|-_{\Sigma, \Omega} \isTerm{x}}\,\Rule{Var}
    \and
    \inferrule{\cdot |-_{\Sigma} A \\ |-_{\Sigma, \Omega}\isTerm{t}}{|-_{\Sigma, \Omega} \isTerm{t \annotate A}}\,\Rule{Anno}
    \and
    \inferrule{|-_{\Sigma, \Omega} \isTerm{t_1} \quad \cdots \quad |-_{\Sigma, \Omega} \isTerm{t_n}}
    {|-_{\Sigma, \Omega} \tmOpts }\,\Rule{Op}
  \end{mathpar}
  \begin{flushright}
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    {\Gamma |-_{\Sigma, \Omega} \tmOpts : \simsub{A}{\rho}}\,\Rule{Op}
    \and \text{for $o\colon \Xi \rhd [\Delta_1]A_1, \ldots, [\Delta_{n}]A_{n} \to A$ in $\Omega$}
  \end{mathpar}
...
\subsection{Bidirectional Binding Signatures and Bidirectional Type Systems} \label{sec:bidirectional-system}
