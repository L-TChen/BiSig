...
\begin{definition}\label{def:bidirectional-typing-derivations}\label{def:mode-derivations}
  ...
  \begin{itemize}
    \item the set of \emph{bidirectional typing derivations} of $\Gamma \vdash t :^\dir{d} A$ for a context $\Gamma$ of typed variables, a raw term $\isTerm{t}$ under $\erase{\Gamma}$, a mode $\dir{d}$, and a type $A$ is defined by \Cref{fig:bidirectional-typing-derivations};
    \item the set of \emph{mode derivations} of $V |- t^\dir{d}$ for a list $V$ of variables, a raw term $\isTerm{t}$ under $V$, and mode $d$ is defined by \Cref{fig:mode-derivations}.
  \end{itemize}
