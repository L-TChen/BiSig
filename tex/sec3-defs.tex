...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    {|-_{\Sigma, \Omega} \isTerm{\tmOp_o(\vec{x}_1.t_1; \ldots; \vec{x}_n.t_n)}}\,\Rule{Op}
  \end{mathpar}
  \begin{flushright}
...
\begin{figure}
  \centering
  \small
  \judgbox{\Gamma |-_{\Sigma, \Omega} \isTerm{t} :^\dir{d} A}{A raw term $t$ is of type $A$ (without type variables) in mode $d$ under $\Gamma$}
  \begin{mathpar}
    \inferrule{(x : A) \in \Gamma}{\Gamma |-_{\Sigma, \Omega} \isTerm{x} :^{\syn} A}\,\SynRule{Var}
    \and
    \inferrule{\Gamma |-_{\Sigma, \Omega} \isTerm{t} :^{\chk} A}{\Gamma |-_{\Sigma, \Omega} (\isTerm{t \annotate B}) :^{\syn}  A}\,\SynRule{Anno}
    \and
