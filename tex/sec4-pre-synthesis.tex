...
Technically, all these can be summarised in one lemma.

\begin{lemma}\label{lem:soundness-completeness}
There is a bidirectional typing derivation of\/ $\Gamma |-_{\Sigma, \Omega} t :^\dir{d} A$ if and only if there are mode and typing derivations of\/ $\erase\Gamma |-_{\Sigma, \Omega} t^\dir{d}$ and\/ $\Gamma |-_{\Sigma, \erase\Omega} t : A$.
\end{lemma}
...
%If\/ $V \not|-_{\Sigma,\Omega} t^\dir{d}$, then $V |-_{\Sigma,\Omega} t^{\dir{d}\,\BFalse\,s}$ for some boolean~$s$.
%\end{corollary}
%
...
%Symmetrically, if $V \not|-_{\Sigma,\Omega} t^\dir{d}$, then $g$~has to be~$\BFalse$ so as not to contradict \cref{thm:Pre?-true}.
%\end{proof}
