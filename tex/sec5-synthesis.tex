...
\LT{perhaps we shall prove it decidable}
...
\begin{definition}
  Let $\rho$ be a partial substitution.
  Then, by an \emph{extension}\/ $\sigma \geq \rho$ we mean that the domain of $\sigma$ contains the domain $\rho$ and\/ $\sigma(x) = \rho(x)$ for every\/ $x$ in the domain of $\rho$; by a \emph{minimal extension}\/ $\bar{\rho}$ of $\rho$ satisfying $P$ we mean an extension $\bar{\rho} \geq \rho$ with $P(\bar{\rho})$ such that $\sigma \geq \bar{\rho}$ whenever $\sigma \geq \rho$ and $P(\sigma)$.
\end{definition}
...
\begin{proof}[Proof of {\Cref{thm:bidirectional-type-synthesis-checking}}]
  ...
  \begin{itemize}
    ...
    \item \ChkRule{Sub}: If $\erase{\Gamma} |-t^{\chk}$ by the rule $\ChkRule{Sub}$, then $\Gamma |- t^{\syn}$ by inversion.
      We show that it is decidable for any $A$ whether $\Gamma |- t \chk A$.
    ...
    \item \Rule{Op}:
      ...
      \begin{claim}\label{lem:args-induction}
        For an argument list $\as = \biargvec$ and any partial substitution $\rho$ from $\Xi$ to $\emptyset$
        \begin{enumerate}
        ...
        \end{enumerate}
      \end{claim}
      Then, case analysis on $\dir{d}$:
      \begin{itemize}
        \item $\dir{d}$ is $\syn$: We apply our claim with the partial substitution $\rho_0$ defined nowhere.
          \begin{enumerate}
            \item If there is no $\sigma \geq \rho$ such that \eqref{eq:claim} holds but for some $A$ we have $\Gamma |- \tmOpts \syn A$, then by inversion we have a substitution $\rho\colon \Sub{\Xi}{\emptyset}$ such that ${\Gamma, \vars{x}_\isTerm{i} : \simsub{\Delta_i}{\rho} |- \isTerm{t_i} \colon \simsub{A_i}{\rho}^{\dir{d_i}}}$ for every $i$.
              Obviously, $\rho$ is an extension of $\rho_0$ such that ${\Gamma, \vars{x}_\isTerm{i} : \simsub{\Delta_i}{\rho} |- \isTerm{t_i} \colon \simsub{A_i}{\rho}^{\dir{d_i}}}$ for every $i$ and it contradicts the assumption that no such an extension exists.

            \item If there exists a minimal $\ext{\rho} \geq \rho_0$ defined on $\synvar(\biargs)$ such that \eqref{eq:claim} holds, then by mode-correctness we have $\ext{\rho}$ is total and
              \[
      ...
      \end{itemize}
      \begin{claimproof}
        For a partial substitution $\rho$, we prove it by induction on the list $\biargvec$:
        \begin{enumerate}
                    ...
                    \begin{enumerate}
                      ...
                      \item If $\Gamma, \vars{x}_\isTerm{m+1} : \simsub{\Delta_{m+1}}{\ext{\rho}} |- \isTerm{t_{m+1}} \syn A$ for some $A$, then by \Cref{lem:unify} we have two cases: %we unify $A$ with $A_{m+1}$ extending $\ext{\rho}$.
                        \begin{itemize}
                          ...
                          \item If there is a minimal $\ext{\ext{\rho}}$ such that $\simsub{A_{m+1}}{\ext{\ext{\rho}}} = A$, then it is now hard to show that $\ext{\ext{\rho}}$ is the minimal extension of $\rho$ such that \eqref{eq:claim} holds for all $1 \leq i \leq m + 1$.
                        \end{itemize}
                    \end{enumerate}
                \end{itemize}
            \end{enumerate}
      ...
      \end{claimproof}
      We now complete the decidability proof.
  \end{itemize}
\end{proof}
While the proof presented above is complex, its formal counterpart essentially serves as two programs for checking and synthesis.
These programs either compute the typing derivation or provide a proof of contradiction.
Each case analysis simply branches based on the outcomes of bidirectional type synthesis and checking for each subterm, as well as the unification.
If a proof of contradiction is not of interest for implementation, the programs can be simplified by disregarding the cases that provide such contradiction proofs.
On the other hand, the contradiction proof contains the information as to why the construction of a derivation failed.
This can potentially be restructured further to provide informative error messages, assisting users in resolving issues with ill-typed terms, instead of merely answering a blatant `no'.
