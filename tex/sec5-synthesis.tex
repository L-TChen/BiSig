...
\end{enumerate*}
Here `input' variables refer to variables in an extension context and in the type of a premise in checking mode.
...
\begin{definition}
  ...
  The subset $\fv(\Gamma)$ of variables in an extension context~$\Delta$ is defined by\/ $\fv(\cdot) = \emptyset$ and\/ $\fv(\Delta, A) = \fv(\Delta) \cup \fv(A)$.
...
\end{definition}
This definition encapsulates the idea that every `input' type variable, possibly derived from a context $\Delta$ or a checking argument $A_n$, must be an `output' variable from $\synvar(\biargvec)$ or, if the rule is checking, belong to the set $S$ of `input' variables in its conclusion.
  ...
  \begin{itemize}
    ...
    \item $\Rule{Op}$: Recall that a derivation of\/ $\Gamma |- \tmOpts \syn A$ contains a substitution $\rho$ from the local context $\Xi$ to concrete types.
      ...
      \begin{enumerate}
        ...
        \item If $\dir{d_{i+1}}$ is~$\syn$, then $\simsub{\Delta_{i+1}}{\rho_1} = \simsub{\Delta_{i+1}}{\rho_2}$ by mode-correctness \Cref{eq:MC2} and induction hypothesis (of the list).
...
Combining the bidirectional synthesiser in \cref{subsec:bidirectional-synthesis-checking} with the mode decorator, soundness, and completeness in \cref{sec:pre-synthesis}, we can now construct a type synthesiser, which generalises \cref{thm:implementation}.
