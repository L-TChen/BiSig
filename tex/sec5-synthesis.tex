...
\begin{proof}[Proof of {\Cref{thm:bidirectional-type-synthesis-checking}}]
  ...
  The last case \Rule{Op} is new and has been discussed informally above.
  \begin{itemize}
    ...
    \item \ChkRule{Sub}: If $\erase{\Gamma} |-_{\Sigma, \Omega}t^{\chk}$ by the rule $\ChkRule{Sub}$, then $\Gamma |-_{\Sigma, \Omega} t^{\syn}$ by inversion.
      ...
      \begin{itemize}
        \item If $\Gamma |/- t \syn C$ for any type $C$ but $\Gamma |- t \chk A$ is derivable, then by inversion $\Gamma |- t \syn B$ for some $B = A$, thus a contradiction.
