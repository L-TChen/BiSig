...
We start with presenting the statement of the decidability of bidirectional synthesis (whose proof will be deferred until \Cref{subsec:bidirectional-synthesis-checking}) and the trichotomy on raw terms in \Cref{subsec:trichotomy}.
We will combine mode preprocessing, soundness, and completeness from \Cref{sec:pre-synthesis} to exhibit the trichotomy as a corollary.
We will then formalise the mode-correctness condition and show the uniqueness of synthesized types in \Cref{sec:mode-correctness}.
